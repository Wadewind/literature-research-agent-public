...
async def test_get_document_returns_overview(client) -> None:
...
async def test_list_elements_returns_locations(client) -> None:
...
async def test_list_elements_filters_by_page_and_type(client) -> None:
    """页码与类型过滤应生效。"""
    test_client, project_id, version_id = _ids(client)
...
async def test_list_elements_rejects_invalid_type(client) -> None:
...
async def test_document_of_other_users_project_returns_404(client) -> None:
...
async def test_unknown_version_returns_404(client) -> None:
    """不存在的版本应返回 404。"""
    test_client = client["client"]
...
async def test_document_not_ready_returns_404(client) -> None:
    ...
    relation_repo = client["relation_repo"]
    paper = create_paper(owner_id="user-1")
    await paper_repo.add(paper)
    version = create_paper_version(
        paper_id=paper.paper_id,
