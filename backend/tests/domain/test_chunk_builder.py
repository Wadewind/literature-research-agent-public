...
def test_groups_adjacent_elements_within_max_tokens() -> None:
    ...
    profile = ChunkProfile(max_tokens=10, overlap_tokens=0, include_section_prefix=False)
...
def test_overlap_carries_whole_elements() -> None:
    ...
    profile = ChunkProfile(max_tokens=10, overlap_tokens=4, include_section_prefix=False)
    elements = [_element(i, text=_words(4)) for i in range(1, 5)]
...
def test_table_and_caption_stay_in_same_chunk() -> None:
    ...
    profile = ChunkProfile(max_tokens=512, overlap_tokens=64, include_section_prefix=False)
    table = _element(
        1,
...
def test_section_heading_becomes_prefix_not_chunk() -> None:
    ...
    profile = ChunkProfile(max_tokens=512, overlap_tokens=64)
