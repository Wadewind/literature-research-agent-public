...
- 日期：2026-08-20
- 决策者：项目维护者
- 修订说明：决策三中“搜索候选只有在人工纳入后才能新增 ProjectPaper”的 Phase 3 行为，已由 [产品决策 0003](0003-phase-3-fixed-review-workflow.md) 修订为“arXiv 前 N 篇自动导入，不设置论文人工筛选”。其余 Project-scoped、归档和历史追溯决策保持有效。
...
- 永久删除延后到 Phase 4，在完整引用图、备份和恢复策略明确后实现；删除 Project 不得连带删除 owner 个人文献库中的 Paper。
...
- Paper 永久删除延后到 Phase 4，只允许在没有 ProjectPaper、Run、Evidence、Citation、Workflow 或 Artifact 引用时执行；Storage 回收使用可观察的异步 GC。

Phase 2 的第一个支撑切片实现 Project 修改/归档/恢复和 Paper 归档/恢复；永久删除不属于 Phase 2。
