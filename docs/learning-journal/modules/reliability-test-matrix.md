...
|---|---|---|---|
| 重复 ARQ Job / 等待后恢复 | `backend/tests/integration/test_queue_worker.py::test_stable_job_id_can_run_again_after_paused_attempt`、`::test_distinct_physical_jobs_create_one_business_effect`（PostgreSQL + Valkey + ARQ Worker 集成）；`backend/tests/application/test_run_execution_service.py::test_execute_duplicate_job_is_skipped`（Application） | 稳定 Job ID 在排队/执行中去重，但 `execute_run` 不保留 ARQ Result；首次 Attempt PAUSED 后同一 ID 可合法再投并形成第二 Attempt。不同物理 Job 的重复投递仍只有一个业务效果 | ARQ 去重只是并发优化，正确性来自 PostgreSQL Run 条件认领；ARQ Result 不是事实来源，不宣称队列 Exactly Once |
...
| Agent Runtime 成功、业务结果 commit/响应丢失 | `backend/tests/integration/test_agent_turn_reliability.py::test_result_commit_crash_reconciles_runtime_and_commits_business_once`、`backend/tests/application/test_agent_turn_executor.py::test_runtime_success_response_loss_reconciles_without_second_execute`（PostgreSQL 故障注入/Application） | 真实结果事务 commit 失败后 Message/candidate/Event 全回滚；新 Attempt 先 reconcile 同一 `turn_run_id`，SUCCEEDED/result_available 直接 collect，不第二次 execute；最终只提交一条 Assistant Message、一份 candidate 和一组成功 Event | Fake Runtime 状态仍在同一进程；证明平台协议和数据库 Effectively Once，不证明跨进程 Runtime 持久化或 Provider Exactly Once |
| Agent 重复 Job / 已有 RUNNING Execution | `backend/tests/integration/test_agent_turn_reliability.py::test_result_commit_crash_reconciles_runtime_and_commits_business_once`、`backend/tests/application/test_agent_turn_executor.py::test_existing_running_execution_retries_without_appending_input`（PostgreSQL/Application） | 业务成功后的 ACK Job 重放被 PostgreSQL 终态认领拒绝，不新增 Attempt/Runtime/Message/candidate/Event；Runtime 已报告 RUNNING 时不调用 execute，不重复追加输入，按 temporary 使用既有 Attempt 预算继续对账 | Fake RUNNING 不会自行跨进程推进；切片 4 需以持久 Checkpoint 验证真实恢复 |
| Agent 取消传播 / Worker 崩溃 | `backend/tests/application/test_agent_turn_executor.py::test_running_cancel_propagates_to_runtime_and_commits_no_result`、`::test_queued_cancel_releases_active_turn_without_starting_runtime`、`backend/tests/integration/test_agent_turn_reliability.py::test_cancel_requested_worker_crash_reconcile_releases_agent_session`、`::test_cancel_runtime_temporary_failure_stops_heartbeat_then_reconciles`（Application/PostgreSQL lease） | QUEUED 取消不启动 Runtime；RUNNING 取消在事务外调用 `cancel_turn`、停止消费流，不 collect 或提交 Assistant/candidate；`cancel_turn` temporary 失败保持 CANCEL_REQUESTED/RUNNING Attempt 并停止心跳，不写 FAILED/RETRY_WAIT，随后 lease 对账收敛 Run/Attempt 并释放 Session | Fake 只证明平台不再发起语义操作；不承诺真实 Provider 已在途调用可以立即撤销 |
| Agent Runtime 子任务清理 / 作用域错配 | `backend/tests/application/test_agent_turn_executor.py::test_status_watch_failure_cancels_and_awaits_runtime_stream`、`::test_outer_cancellation_cleans_runtime_stream_and_status_watcher`、`::test_runtime_scope_mismatch_is_permanent_and_persists_no_result`（Application/PostgreSQL） | 状态 watcher 异常和外层 task 取消都会 cancel+await Runtime consumer/watcher；reconciliation/result 的 Turn、Session、Binding 六项错配在业务提交前统一 permanent 失败，不保存错误 Binding/Message/candidate，Attempt 只记录安全 code/描述 | Fake 证明协调层无遗留 consumer 和平台闭包校验，不证明远端 Provider 已撤销收到的请求 |
